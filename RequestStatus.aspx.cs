...
public partial class RequestStatus : System.Web.UI.Page
{
    ...
    protected void TextBoxTrack_TextChanged(object sender, EventArgs e)
    {
    ...
    protected void GridViewTrack_SelectedIndexChanged(object sender, EventArgs e)
    {
    ...
    if (TextBoxUID.Text == "")
        ...
        else
        {
            ...
            if (temp1 >= 1)
            {
                string checkApprove = "select Principal from LeaveRequest where Username='" + Session["User"] + "' and UID='" + TextBoxUID.Text + "'";
                ...
                {
                    Response.Write("<script>alert('Sorry!!Your Request is Currently Pending At Principal')</script>");
                }
            }
            else
            {
